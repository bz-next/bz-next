...
void BulletObjectBuilder::setTeam(TeamColor tc) {
    _tc = tc;
}

void BulletObjectBuilder::setDrawableGroup(DrawableGroup3D* dgrp) {
    _dgrp = dgrp;
}

void BulletObjectBuilder::prepareMaterials() {
    ...
    std::string bulletMatName = "_" + Team::getImagePrefix(_tc)  + "Bullet";
    // Check if it already exists. If not, add it.
    if (MAGNUMMATERIALMGR.findMaterial(bulletMatName) == NULL) {
        auto* bulletmat = new MagnumBZMaterial();
