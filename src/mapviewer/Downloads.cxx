...
CachedTexture::CachedTexture(const std::string &texUrl) : cURLManager()
{
    ...
    if (pos != std::string::npos) {
        Magnum::Warning{} << "Rewriting to https://";
        httpsURL.replace(pos, to_replace.length(), "https://");
    }

    setURL(httpsURL);
    url    = texUrl;    // Cache directory is /http/ not https...

    Magnum::Warning{} << "https url is" << httpsURL.c_str();
    ...
    else
    {
        ...
        if (cached)
        {
            ...
            Magnum::Warning{} << "Cached condition";
        }
        addHandle();
    }
}
...
void CachedTexture::finalization(char *data, unsigned int length, bool good)
{
    ...
    if (good)
    {
        if (length)
        {
            Magnum::Warning{} << "Has length" << length;
...
void CachedTexture::collectData(char* ptr, int len)
{
    ...
    Magnum::Warning{} << "Calling collectData";
    cURLManager::collectData(ptr, len);
    Magnum::Warning{} << "Called collectData";
    byteTransferred += len;
